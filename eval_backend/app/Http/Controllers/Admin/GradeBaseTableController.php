...
class GradeBaseTableController extends Controller
{
    use Tool;
    use Evaluation;
    protected  $model = 'App\Models\GradeBaseTable';  // 当前模型
    protected  $fillable = [];  // 当前模型可以修改和新增的字段
    protected  $resource = 'App\Http\Resources\GradeBaseTable'; // 显示个体资源
    protected  $resourceCollection = 'App\Http\Resources\GradeBaseTableCollection'; // 显示资源集合
...
//        1. 获取前端数据
        ...
        $grade = json_decode(request('grade', null));
        $value = json_decode(request('value', null));
        $mark = request("mark", null);
        $group_id = $grade[0]->group_id;
        $result = [
            "grade" => [],
            "value" => [],
        ];

        DB::transaction(function() use($grade, $value, &$result, &$mark, $group_id){
            $arr1 = [];
            $arr2 = [];
            if (is_null($mark)) {
                DB::beginTransaction();
                $old_mark = DB::table("grade_base_tables")->where("group_id", $group_id)->select(["mark"])->groupBy("mark")
                    ->orderBy("mark", "desc")->lockForUpdate()->value("mark");
                if (is_null($old_mark)){
                   $mark = 1;
                } else {
                    $mark = $old_mark + 1;
                ...
                DB::table("grade_base_tables")->insert($arr1);
                ...
                DB::table("value_base_tables")->insert($arr2);
                DB::commit();
            } else {
                foreach ($grade as &$item) {
                    $item = (array)$item;
...
//            foreach ($grade as &$item) {
//                $id = DB::table("grade_base_tables")->insertGetId($item);
//                $analysis = DB::table("grade_analysis_tables")->get();
//                $tmp = [];
...
//                };
//                for($i = 1; $i<= 20; $i++){
//                    if (array_key_exists("name".$i,$item)) {
//                        foreach($tmp as $k => $v){
//                            if ($v['topic_id'] === $item['topic_id'] && $v['content_id'] === $item['content_id']
//                                && $v['group_id'] === $item['group_id'] && $v["grade_name"] === $item["name".$i]){
//                                $tmp[$k]["name".$i] = (int)$tmp[$k]["name".$i] + 1;
//                                DB::table('grade_analysis_tables')->where('id',$tmp[$k]['id'])->update($tmp[$k]);
//                            }
//                        };
//                    } else {
//                        break;
